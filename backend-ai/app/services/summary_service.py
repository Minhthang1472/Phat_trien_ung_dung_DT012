...
class SummaryService:
    ...
    def summarize_transcript(self, full_text: str) -> dict:
        """
        Gửi transcript bài giảng để tóm tắt các ý chính và câu hỏi ôn tập 4 phần:
        1. Tổng quan bài học (summary)
        2. Điểm cốt lõi (key_points)
        3. Thuật ngữ / công thức quan trọng (formulas_and_terms)
        4. Bộ câu hỏi trắc nghiệm tự ôn tập (quiz)
        """
        ...
        try:
            ...
            # Làm sạch nếu model trả về markdown ```json ... ```
            ...
            logger.error(f"Lỗi khi gọi LLM tóm tắt: {e}")
            return {
